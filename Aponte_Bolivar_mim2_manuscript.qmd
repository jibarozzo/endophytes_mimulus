...
p_final_model <- ggplot(data = merged_data, aes(x = dbRDA1, y = dbRDA2)) +
  ...
  geom_segment(arrow_map, linewidth = rel(.8), data = arrow_scores[1:2, ], color = "black", arrow = arrowhead) +
  ...
  geom_text(label_map, size = rel(4), data = A4[[1]], fontface = "bold", show.legend = FALSE, label = c("Elev."), nudge_x = 0.65) +
  ...
  scale_color_manual(labels = c("*M. laciniatus*","*M. nasutus*", "*M. guttatus*"), values= met.brewer("Isfahan2", n = 3, type = "discrete", direction = -1)) +
...
ggsave(filename=file.path(path, "figures/dbRDA_elevation_species.png"), plot = p_final_model, dpi=300, units=c("mm"), width=250, height=150)
...
ggsave(filename = file.path(path, "figures/supp_fig4.png"), plot = lma_elevation, dpi=300, units=c("mm"), width=300, height=250)
```
...
aci_elevation
ggsave(filename = file.path(path, "figures/supp_fig5.png"), plot = aci_elevation, dpi=300, units=c("mm"), width=300, height=250)
```
...
ggsave(filename = file.path(path, "figures/supp_fig6.png"), plot = lbi_elevation, dpi=300, units=c("mm"), width=300, height=250)
```
...
ggsave(filename = file.path(path, "figures/supp_fig7.png"), plot = lps_elevation, dpi=300, units=c("mm"), width=300, height=250)
```
...
lt_elevation <- ggarrange(lt_cont, lt, ncol = 1, nrow = 2, 
                           ...
                           heights = c(1, 1))
lt_elevation
ggsave(filename = file.path(path, "figures/supp_fig8.png"), plot = lt_elevation, dpi=300, units=c("mm"), width=300, height=250)
```
