...
data.pca <- leaf_traits_noB |>
  na.omit() |>
  select(c(19:23)) #Selecting only the log-transformed leaf trait columns. I use the log transformed data due to our findings of non-normality in the data. See this post for a hot take: https://stats.stackexchange.com/questions/164381/why-log-transforming-the-data-before-performing-principal-component-analysis
...
Visual assesment of samplint depth.
...
cover_goods|>
  filter(n_seqs > 5000) |> #Filtering samples with more than f000 reads
...
I are calculating some curves to see if we have enough sampling effort.
...
# Vegan
# Rarefaction curve
vegan::rarecurve(t(asv_matrix), step=50, cex=0.5, xlab = "Sample Size", ylab = "Species", label = TRUE, tidy = FALSE) # rrarefy generates one randomly rarefied community data frame or vector of given sample size. 
...
# Select sample names
seq_sample_names <- sample_names(pseq_rrf1[[1]]) # Sample names

# Randomize sample names
...
# Select a random vector of names
rand_mat <-  matrix(NA_character_ , nrow = 95, ncol = 1)
set.seed(443)
for (i in 1:95) {
  rand_mat[,i] <- sample(rand_matrix[,i], 1, replace = FALSE)
}
...
#saveRDS(plant_traits_rf, file = "clean_data/statistics/plant_traits_rf.rds") # Saving rarefied data frame
#write.csv(plant_traits_rf, file = "clean_data/statistics/plant_traits_rf.csv") # Saving rarefied data frame as csv

#Loading rarefied data frame
plant_traits_rf <- readRDS("clean_data/statistics/plant_traits_rf.rds") # Loading rarefied data frame
```

I now have a data frame with 98 samples with rarefied alpha diversity indices and plant traits. I will use this for downstream analyses.
...
test <- sample_data(pseq_rrf1[[1]]) |> # Rarefied phyloseq object
  rownames() |>
  as.character()

# Check if the vectors are identical
identical(test, names_rarefy)
...
# Shannon diversity or hill number 1
# Linear model summary
summary(lm(Shannon ~ Elevation_m, data = plant_traits_rf))
...
# Plot plant_traits$Species[plant_traits$Species != "B"]
# Plot
...
# Phylum level per elevation catergory and species
p1 <- ggplot(ps_melt |>
...
# Phylum le per sample

p2 <- plot_bar(microbiome::transform(ps_clean_3, "compositional")) +
  guides(fill = guide_legend(title = "Phylum"), color = guide_legend(title = "Phylum")) +
  geom_bar(aes(fill = Phylum, color = Phylum), stat = "identity", position = "fill") +
  scale_fill_manual(labels=c('Ascomycota', 'Basidiomycota', 'Chytridiomycota', 'Fungi_ord_Incertae_sedis', 'Mortierellomycota', 'Olpidiomycota'),values= met.brewer("Lakota", n = 6 , type = "discrete", override.order = TRUE)) +
  scale_color_manual(labels=c('Ascomycota', 'Basidiomycota', 'Chytridiomycota', 'Fungi_ord_Incertae_sedis', 'Mortierellomycota', 'Olpidiomycota'), values = met.brewer("Lakota", n = 6, type = "discrete", override.order = TRUE)) +
  theme_classic(base_size = 14) +
...
# 
# # Perform ANOVA
# anova <- aov(mean_abundance ~ sample_Species, data = ps_melt)
# 
# # Tukey's test for multiple comparisons
# tukey <- glht(anova, linfct = mcp(sample_Species = "Tukey"))
# cld <- cld(tukey, level = 0.05, decreasing = TRUE)
# 
# # Extract letters from Tukey's test
# cld_table <- as.data.frame(cld[["mcletters"]][["Letters"]])
# colnames(cld_table) <- "alphabet"
# 
# # Join letters with the main data
# cld <- cld_table |> rownames_to_column(var = "sample_Species")
# ps_melt <- ps_melt |> left_join(cld, by = "sample_Species")
# 
# # Plot bar chart with ggplot2
# p2 <- ggbarplot(ps_melt, x = "Phylum", y = "mean_abundance", 
#           add = c("mean_se"),
#           color = "sample_Species", 
#           position = position_dodge(0.8),
#           error.plot = "errorbar") 
# 
#  p2 + geom_text(aes(x = Phylum, 
#                                #y = mean_abundance + se + 1,
...
#   group_by(Phylum, sample_Species, alphabet) |>
#   summarise(mean_abundance = mean(Abundance), se = sd(Abundance)/sqrt(length(Abundance)))
# 
# # Another way to plot with ggplot2
# ggplot(ps_melt, aes(x = factor(Phylum), y = mean_abundance, fill = sample_Species)) +
#   geom_bar(stat = "identity", position = position_dodge(0.8)) 
#   geom_errorbar(aes(ymin = mean_abundance - se, ymax = mean_abundance + se), position = position_dodge(0.8), width = 0.25, check_overlap = TRUE) 
#   geom_text(aes(y = mean_abundance + se +1, label = alphabet),  hjust = -5) +
#   labs(title = "Bar Plot of Mean Abundance by Species",
#        x = "Sample Species",
#        y = "Mean Abundance") +
#   theme_minimal()
# 
# # Plot using microbiome package
# plot_bar(microbiome::transform(ps_clean_3, "compositional"), "Phylum", fill = "sample_Species") +
#   geom_bar(aes(fill = Phylum, color = Phylum), stat = "identity", position = "dodge")
# 
# # Statistical comparisons
# stat_compare_means(aes(group = "sample_Species"), label = "p.signif", method = "t.test", hide.ns = TRUE)
# 
# # Wilcoxon rank-sum test for each Phylum
...
#               conf.level = 0.95)
# 
# # Another way to perform Tukey's test
...
m0 <- dbrda(asv_avgdist ~ 1, data = nmds_traits) #Model with intercept only.
m0
m0 <- na.omit(m0)
m1 <- dbrda(asv_avgdist ~ ., data = nmds_traits, na.action = na.omit) # Model with all explanatory variables.
m1 <- 

# Model with species, site, leaf traits and elevation. ####
m2 <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_cat + Site + Species,  data = nmds_traits, na.action = na.omit)
m2
m2 <- na.omit(m2)
Anova(m2)
...
```{r, m2dbRDA}
m2dbRDA <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_m, distance = "bray", data = nmds_traits, na.action = na.omit)
...
```{r, adonis2}
set.seed(123)
perma <- adonis2(asv_avgdist ~ logACI + 
                   logLT + 
                   logLPS + 
                   logLMA + 
                   logLBI + 
...
set.seed(123)
# By Species
beta <- betadisper(as.dist(asv_avgdist), nmds_traits$Species, type = "centroid", sqrt.dist = FALSE)
beta
anova(beta)
...
# By Elevation
beta2 <- betadisper(as.dist(asv_avgdist), nmds_traits$Elevation_cat, type = "centroid", sqrt.dist = FALSE)
anova(beta2)
TukeyHSD(beta2)
...
```{r, SIMPER}
(sim <- with(nmds_traits, simper(asv_avgdist, Elevation_cat, permutations = 999)))
summary(sim)
```
