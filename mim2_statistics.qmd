...
save(m1_lts, file = "clean_data/statistics/m1_lts.rda")
load(file = "clean_data/statistics/m1_lts.rda")
...
anova_m1_lts_margin <-
  anova.cca(m1_lts,
            ...
            permutations = 999,
            parallel = 8)
save(anova_m1_lts_margin, file = "clean_data/statistics/anova_m1_lts_margin.rda")
load("clean_data/statistics/anova_m1_lts_margin.rda")
...
save(anova_m1_lts_term, file = "clean_data/statistics/anova_m1_lts_term.rda")
load("clean_data/statistics/anova_m1_lts_term.rda")
...
### Preparing data for modelling GLMs
...
dbrda_traits_imp <- dbrda_traits |>
  mutate(across(c(, 8:12), ~ ifelse(is.na(.), median(., na.rm = TRUE), .)))
...
dbrda_traits_imp <- left_join(dbrda_traits, mean_bray, by = c("X"= "Unique_ID"))
...
# Model with intercept only
B0 <- lm(mean_bray ~ 1, data = plant_traits_MB, na.action = na.exclude)
anova(B0)
AIC(B0)
plot(B0)
...
plot(x=predict(B1), y=plant_traits_MB$mean_bray,
...
# predicted vs. actual values
plot(x=predict(B2), y=plant_traits_MB$mean_bray,
     xlab='Predicted Values',
     ylab='Actual Values',
...
B3 <- lme(
  ...
  random = ~ 1 | Site,
  method = "ML",
  data = na.omit(plant_traits_MB)) 

Anova(B3, type = 3)
AIC(B3) # -8688.862 Increased AIC score compared to B1 and B2.
...
# Plots
plot(x=predict(B3), y=plant_traits_MB$mean_bray,
     xlab='Predicted Values',
     ylab='Actual Values',
...
# Plots
plot(x=predict(B4), y=plant_traits_MB$mean_bray,
     xlab='Predicted Values',
     ylab='Actual Values',
...
# logLPS eliminated
```
The B4AIC model has a lower AIC score. We seem to have hit a good model. The fixed terms are `mean_bray ~ logACI + logLT + logLMA + logLBI`. Let's take that an model random effects taking into account variance structure. 
...
Anova(B1REM, type = 3)
AIC(B1REM) # -8609.769 This is a higher score than B4AIC.
...
Anova(B1_gls, type = 3)
AIC(B1_gls) # -6970.8 is a higher AIC score. Site as a random effect is important.

plot_model(B1REM, type = "diag")

# Model comparison
model.sel(B1, B2, B3, B1REM, B1_gls)[, c("df", "logLik", "AICc", "delta")]
...
Anova(B3REM, type = 3)
AIC(B3REM) # -9467.585 is  a lower AIC score than B1REM
...
plot(B3REM)
B3REM$
plot(x=predict(B3REM), y=plant_traits_MB$mean_bray,
     xlab='Predicted Values',
     ylab='Actual Values',
...
The B3REM model has the lowest AIC score yet. The the fixed terms are `mean_bray ~ logACI + logLT + logLPS + logLMA + logLBI,` with a variance structure `weights = varIdent(form = ~ 1 | Site)`. The random effects are nested `random = 1 | Site/Species`. It seems to be a good fit for the data. 
...
B4REM <- lme(
  mean_bray ~ Elevation_m + logACI + logLT + logLMA + logLBI,,
...
library("MuMIn")
model.sel(B1, B2, B3, B1REM, B3REM, B4REM, B5REM)[, c("df", "logLik", "AICc", "delta")]
