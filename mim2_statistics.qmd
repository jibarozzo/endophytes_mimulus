...
plant_traits <- ps_clean_3 |> #Estimating community richness and shannon diversity from phyloseq object
  estimate_richness(split = TRUE, measures = c("Observed","Shannon", "Simpson", "InvSimpson")) |>
       #  Simpson = 1 / Simpson) |>
  rownames_to_column() |>
  rename(Unique_ID = rowname) |>
  right_join(plant_traits, by = "Unique_ID") |>
  relocate(Observed, Shannon, Simpson, InvSimpson, .after = logLMA) |>
  rename(Simpson_1D = Simpson)
...
Here we are going to explore the relationship between elevation and FEF community composition through richness and species diversity.
Let's calculate Hill numbers for the FEF community and plot them against elevation.
First let's create a distance matrix.
```{r, dist_matrix}
#| echo: false
#| eval: true
#| tidy: true
...
```{r, hill_numbers}
ggplot(plant_traits |>
  filter(!Species == "B"), aes(x = Elevation_cat, y = InvSimpson, shape = Species)) +
  geom_jitter(aes(fill = Elevation_cat))
```

#### Elevation_m vs. Shannon diversity and Richness
...
rich_diversity <- ggplot(data = plant_traits  |>
  ...
  stat_regline_equation(label.y = 45, label.x = 1900, 
   ...
   scale_color_manual(legend_title, labels = c("*M. laciniatus*", "*M. nasutus*", "*M. guttatus*"), 
                      values = met.brewer("Archambault", n = 4, type = "discrete", direction = c(1))) + 
   labs(y = "Richness", x = "Elevation_m (m.a.s.l.)", col = "") + 
   theme_classic(base_size = 12) +
   theme(legend.title = element_text(face = "bold"),
        panel.border = element_rect(linetype = "blank", fill = NA),
        legend.text = element_markdown(size = 12), #Make legend text italic
...
#scale_shape_manual(values=c(1:24)) +
  scale_color_manual(values= met.brewer("Archambault", n = 4, type = "discrete"))+
  geom_smooth(method=lm, se = F) +
 # stat_regline_equation(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = "~~~~"))) +
  geom_smooth(method = lm, se = T , level = 0.95, na.rm = F, color = "black") +
...
ps_melt <- psmelt(ps_clean_3) # Melting data to create accurate relative abundance plot
p1 <- ggplot(ps_melt |>
...
#my_colors <- MetBrewer::met.brewer("Archambault",4), override.order = TRUE
...
pp <-  p +  geom_violin(aes(fill=Species, color = Species)) +
  ...
  scale_fill_manual(labels=c('*M. guttatus*', '*M. laciniatus*', '*M. nasutus*'),values= met.brewer("Archambault", n = 3, type = "discrete")) +
  scale_color_manual(labels=c('*M. guttatus*', '*M. laciniatus*', '*M. nasutus*'),values= met.brewer("Archambault", n = 3, type = "discrete")) +
  theme_bw() +
  theme(panel.grid.major = element_line(colour = "grey90")) +
...
```{r, Distance Matrix}
asv_dist <- vegdist(t(dist_matrix), method="bray", binary=FALSE)
...
m2 <- capscale(asv_e.dist ~ Species + ACI + Thickness + Toughness + LMA + LBI + Elevation_m,  data = nmds.traits, na.action = na.omit)
m2
m2 <- na.omit(m2)
Anova(m2)
# Model with just traits ####
# No species, Trial_type and E_load
m3 <- capscale(asv_e.dist ~ ACI + Thickness + Toughness + LMA + LBI + Elevation_m, data = nmds.traits, na.action = na.omit)
m3
Anova(m3)
...
m3dbRDA <- capscale(asv_e.dist ~ ACI + Thickness + Toughness + LMA + LBI + Elevation_m,  distance = "bray", data = nmds.traits, na.action = na.omit)
summary(m3dbRDA)
plot(m3dbRDA)
...
p_m3dbrda <- p_m3 +
  ...
  ylab(label = paste("CAP2 (", round(B$concont$importance[2,2]*100, digits = 1), "%)", sep="")) + scale_color_manual(labels=c('*M. guttatus*', '*M. laciniatus*', '*M. nasutus*'),values = met.brewer(name = "Archambault", n = 4, type="discrete")) +
...
cotu <- left_join(tpqmax, ftraits2, by = "Unique_ID") |>
  mutate(Elevation_cat = cut(Elevation_m, breaks = breaks, labels = labels)) |>
    select(!c(ACI, 
            Thickness,
            Toughness, 
            LMA, 
            LBI, 
            log10LBI, Elevation_m, 
            ImageJ_1, 
            ImageJ_2)) |>
