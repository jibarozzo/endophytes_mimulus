...
betamatPCoA <- function(x, transformation="hellinger", dsim="bray", md= c("pca", "pcoa"), ...){
...
pcoa_object <- betamatPCoA(pseq_rrfb, transformation = "hellinger", dsim = "bray", md = "pcoa")

pcoa_df <- data.frame(pcoa_object$points) |>
rownames_to_column(var = "Unique_ID")

# Merge scores with trait data
merged_data <- right_join(pcoa_df, plant_traits_MB, by = c("Unique_ID" = "X"))

ggplot() +
geom_point(data = merged_data, aes(x = Dim1, y = Dim2, color = Species)) +
# stat_ellipse(aes(color = Species),
#                geom = "path",
#                linewidth = 1.7,
#                position = "identity",
