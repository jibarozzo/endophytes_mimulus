...
m0 <- dbrda(rrfy_hell_matrix ~ 1, method = "bray", data = plant_traits_MB, na.action = na.omit) #Model with intercept only.
m0 <- na.omit(m0)

m1 <- dbrda(rrfy_hell_matrix ~ ., method = "bray", data = plant_traits_MB, na.action = na.omit) # Model with all explanatory variables.
m1 <- na.omit(m1)

# Model with species, site, leaf traits and elevation. ####
m2 <- dbrda(rrfy_hell_matrix ~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_m + Site + Sample_Date+ Species, method = "bray", data = plant_traits_MB, na.action = na.omit)
