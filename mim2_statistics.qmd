...
# This is the Bray-Curtis dissimilarity matrix can be calculated when plotted. 
#asv_vegdist <- vegdist(rrfy_hell_matrix, method = "bray") #Samples as rows, ASVs as columns

# Previous dissimilarity matrix
# Mean dissimilarity matrix from rarefied data sets
# set.seed(433)
...
# Calculated with the dbrda function from the vegan package downstream.
...
identical(sort(test), sort(names_rarefy)) # TRUE
...
# Adding column and rownames
rownames(geo_distm) <- rownames(rrfy_hell_matrix)
colnames(geo_distm) <- rownames(rrfy_hell_matrix)

saveRDS(geo_distm, file = "clean_data/statistics/geo_distm.rds") # Saving rarefied phyloseq object
...
#ggsave(file.path(path, "figures/relabun_barplot.png") , plot = p2, dpi=300, units=c("mm"), width=200, height=150)
```
...
# NMDS Aesthetics ####
#Subset data set for NMDS and dbRDA plot. See Cleaning Shaping chunk for more details.

 NMDS <- ggplot(data.scores, aes(NMDS1, NMDS2, color = ordi_traits$Species))+ 
...
After some thought, and seeing how unrealistic using the same trait data from host species applied to all rarefied datasets results in a model with all terms significant, I decided to use simulate data based on the same distribution and characteristics of the original data and compare results.

```{r}
# Simulating data
sim_ACI <- rnorm(4200, mean = mean(leaf_traits_noB$ACI, na.rm = T), sd = sd(leaf_traits_noB$ACI, na.rm = T)) 
sim_LT <- rnorm(4200, mean = mean(leaf_traits_noB$LT, na.rm = T), sd = sd(leaf_traits_noB$LT, na.rm = T))
sim_LPS <- rnorm(4200, mean = mean(leaf_traits_noB$LPS, na.rm = T), sd = sd(leaf_traits_noB$LPS, na.rm = T))
sim_LMA <- rnorm(4200, mean = mean(leaf_traits_noB$LMA, na.rm = T), sd = sd(leaf_traits_noB$LMA, na.rm = T))

sim_data <- lapply(leaf_traits_noB[,13:16], function(trait) {
    rnorm(n, mean = mean(trait, na.rm = TRUE), sd = sd(trait, na.rm = TRUE))
  })

# Adding to the data frame
sim_traits <- dbrda_traits |>
  mutate(logACI = log10(sim_ACI), logLT = log10(sim_LT), logLPS = log10(sim_LPS), logLMA = log10(sim_LMA))
```
...
m2_hell <- dbrda(dbrda_hell_matrix ~  logACI + logLT + logLPS + logLMA + Elevation_m + Sample_Date + Site + Species,  
            distance = "bray",
            dfun = vegdist,
            data = dbrda_traits,
            parallel = 8, #Passes parallelization to metaMDS function
...
All of the sampling dates are redundant. I will remove `Sampling_Date` term. Elevation alsohas an extremely high vif, but it is part of the question. Let's keep it for a minute. 
```{r}
m3_hell <- dbrda(dbrda_hell_matrix ~  logACI + logLT + logLPS + logLMA + Elevation_m + Site + Species,  
            distance = "bray",
            dfun = vegdist,
            data = dbrda_traits,
            parallel = 8, #Passes parallelization to metaMDS function
...
vif.cca(m3_hell)
inertcomp(m3_hell, display = c("sites"),
    unity = F, proportional = TRUE)
```
...
```{r}
# Real data
m4_hell <- dbrda(dbrda_hell_matrix ~  logACI + logLT + logLPS + logLMA + Elevation_m + Species,  
            distance = "bray",
            dfun = vegdist,
            data = dbrda_traits,
            parallel = 8, #Passes parallelization to metaMDS function
...
vif.cca(m4_hell) # Excellent VIFs
summary(m4_hell)
...
# Simulated data
m4_sim <- dbrda(dbrda_hell_matrix ~  logACI + logLT + logLPS + logLMA + Elevation_m + Species,  
            distance = "bray",
            dfun = vegdist,
            data = sim_traits,
            parallel = 8, #Passes parallelization to metaMDS function
            na.action = na.omit)
vif.cca(m4_sim) # Excellent VIFs
summary(m4_sim)
save(m4_sim, file = "clean_data/statistics/m4_sim.rda")
```


##### More model selection

Continuation of model selection now that I removed redundant terms. 

```{r}
# Anovas
set.seed(123)
anova.m4hell <- anova.cca(m4_hell, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
save(anova.m4hell, file = "clean_data/statistics/anova_m4hell.rda")

set.seed(123)
anova_m4sim <- anova.cca(m4_sim, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
save(anova_m4sim, file = "clean_data/statistics/anova_m4sim.rda")
```

##### Model validation

Compare synthetic model to real data model.

```{r}
# Real matrix
dbrda_real_traits <- plant_traits |>
  filter(Unique_ID %in% final_names_methodB) |>
  select(logACI, logLT, logLPS, logLMA, Elevation_m, Species)

m1_real <- dbrda(t(asv_matrix) ~ ., 
            distance = "bray", 
            dfun = vegdist,
...
vif.cca(m1_real)
anova.cca(m1_real, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
plot(m1_real)
plot(m4_sim)
```
...
```{r}
# Remove logLBI p-value = 0.869
m3 <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + logLMA + Elevation_m + Sample_Date + Site + Species, method = "bray", data = ordi_traits, na.action = na.omit)
# Anovas
anova(m3, by = "margin") # test for sign. environ. variables

# Remove logLMA p-value = 0.754 
m4 <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + Elevation_m + Sample_Date + Site + Species, method = "bray", data = ordi_traits, na.action = na.omit)
# Anovas
anova(m4, by = "margin") # test for sign. environ. variables

#Remove logLT p-value = 0.197
m5 <- dbrda(asv_avgdist ~ logACI + logLPS + Elevation_m + Sample_Date + Site + Species, method = "bray", data = ordi_traits, na.action = na.omit)
# Anovas
anova(m5, by = "margin") # test for sign. environ. variables

#Remove Sample_Date p-value = 0.207
m6 <- dbrda(asv_avgdist ~ logACI + logLPS + Elevation_m + Site + Species, method = "bray", data = ordi_traits, na.action = na.omit)
# Anovas
anova(m6, by = "margin") # test for sign. environ. variables

#Remove logLPS p-value = 0.124
m7 <- dbrda(asv_avgdist ~ logACI + Elevation_m + Site + Species, method = "bray", data = ordi_traits, na.action = na.omit)
# Anovas
anova(m7, by = "margin") # test for sign. environ. variables
```
...
set.seed(123)
# By Species
beta <- betadisper(vegdist(dbrda_hell_matrix, method = "bray"), # Matrix of Hellinger transformed data rarefied 
...
# By Elevation
beta2 <- betadisper(as.dist(asv_avgdist), ordi_traits$Elevation_cat, type = "centroid", sqrt.dist = FALSE)
anova(beta2)
TukeyHSD(beta2)
