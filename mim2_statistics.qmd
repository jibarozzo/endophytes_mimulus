...
### PCoA Simple Linear regressions

```{r}
# PCoA regressions
# PCoA1 vs alpha diversity

alpha_pcoa1 <- lm(hill_1 ~ Dim1, data = pcoa_merged_data)
plot(alpha_pcoa1)
...
alpha_pcoa <- ggplot(data = pcoa_merged_data, aes(y = hill_1, x = Dim1, color = Species)) +
  ...
  labs(y =  "Alpha diversity (\U1D492 = 1)",
       x = paste("PCoA1 (", pretty_percent[1], "%)", sep = "")) +
  scale_color_manual(
    labels = c("*M. laciniatus*", "*M. nasutus*", "*M. guttatus*"),
    values = met.brewer(
      "Isfahan2",
...
M2AIC <- stepAIC(update(M2, . ~ .), direction = "both", k = 2)
M2AIC$anova
...
anova(M3) #Adding Site as a random effect INCREASES AIC score.
Anova(M3, type = 3)
AIC(M3) # -14796.77

# Residuals histograms and plots ####
M2_resid <- resid(M3, type = "response")
M2_fitted <- fitted(M3)
...
anova(M4) # Nesting Species in Site DECREASES AIC score.
AIC(M4) # -15633.11
...
# Stepwise selection of terms
drop1(M4, . ~ ., test = "Chisq", trace = TRUE)
```
...
Anova(M1REM, type = 3)
AIC(M1REM) # -7849.631 This is a higher score than M1.
...
Anova(M1_gls, type = 3)
AIC(M1_gls) # -6815.453 is a higher AIC score. Site as a random effect is important.

plot_model(M1REM, type = "diag")

# Stepwise selection of terms
M1REM_drop <- drop1(M1REM, . ~ ., test = "Chisq", trace = TRUE)
```
...
Anova(M3REM, type = 3)
AIC(M3REM) # -9527.507 is  a lower AIC score than M1REM

# Residuals histograms and plots ####
M3REM_resid <- resid(M3REM, type = "response")
M3REM_fitted <- fitted(M3REM)
...
# Plots
plot(M3REM)
plot_model(M3REM, type = "diag")

# Stepwise selection of terms
drop1(M3REM, . ~ ., test = "Chisq", trace = TRUE)
```

The M3REM model has the lowest AIC score yet. The the fixed terms are `mean_bray ~ logACI + logLT + logLPS + logLMA + logLBI,` with a variance structure `weights = varIdent(form = ~ 1 | Site)`. The random effects are nested `random = 1 | Site/Species`. It seems to be a good fit for the data. 
...
anova(M4REM) # Adding Species as a random effect DECREASES AIC score.
Anova(M4REM, type = 3)
AIC(M4REM) # -10690.23 the lowest AIC score

# Residuals histograms and plots ####
M4REM_resid <- resid(M4REM, type = "response")
M4REM_fitted <- fitted(M4REM)
...
library("MuMIn")
model.sel(M1, M2, M3, M1REM, M3REM, M4REM, M5REM)[, c("df", "logLik", "AICc", "delta")]
```
