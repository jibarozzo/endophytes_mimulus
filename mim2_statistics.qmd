...
pseq_rrf <- phyloseq_mult_raref(ps_clean_3,
                                ...
                                replace = FALSE,
                                multithread = TRUE)
alpha_df <- alphadivDF(pseq_rrf5, diversity = c("shannon", "simpson", "invsimpson")) # Alpha diversity indices
repotu_df(pseq_rrf5) # Report of the rarefaction process
div_values <- vegan::diversity(t(repotu_df(pseq_rrf5)), index = "shannon")
...
alphadivDF <- function(x, diversity="all", zeroes=TRUE){
  md <- sample_data(x[[1]])
  div <- microbiome::diversity(t(repotu_df(x)), index=diversity)
...
alphadivDF(pseq_rrf5, diversity = c("shannon", "simpson", "invsimpson")) # Alpha diversity indices

ggplot(alpha_df, aes(x = Species, y = simpson_, color = Species)) +
  geom_jitter()+
    geom_crossbar(stat = "summary",
                fun = mean,
                fun.max = mean,
  ...
  #       axis.text.x = element_text(size = 14),
  #       legend.position = "bottom") +
  # labs(y = "Diversity Index", x = "Elevation (m.a.s.l.)") +
  # scale_y_continuous(expand = c(0.07, 0.07)) +
  # scale_color_manual(labels =c('*M. laciniatus*', '*M. nasutus*', '*M. guttatus*'),
  #                   values = met.brewer("Isfahan2", n = 4, type = "discrete", direction = c(-1)))
