...
We are calculating some curves to see if we have enough sampling effort. 
```{r, rarefaction}
set.seed(123)
summarize_phyloseq(ps_clean_3)
# Rarefaction curve
rarecurve(asv_matrix, step=50, cex=0.5, xlab = "Sample Size", ylab = "Species", label = TRUE, tidy = FALSE)

# Species accumulation curve
accum_curve <- specaccum(asv_matrix, method = "random", permutations = 1000, cex = 0.5)
plot(accum_curve, col = "blue", ci = 1, pch = 16, main = "Species Accumulation Curve", xvar = "sites", xlab = "Number of Samples", ylab = "Number of Species")

```

It seems we sampled enough, based on the species accumulation curve. The rarefaction curve also shows that species richness is not increasing with greater sequencing depth. Only ASV_1 exceeds 500,000 rarefied sequences. 
