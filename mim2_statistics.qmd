...
m2 <- dbrda(dbrda_hell_matrix ~ Elevation_m + Site + Sample_Date + Species,  
            distance = "bray",
            dfun = vegdist,
            data = dbrda_traits,
            parallel = 8, #Passes parallelization to metaMDS function
...
#Anovas for m2
anova.cca(m2, by = "margin", permutations = 99, parallel = 4)
anova(m2, by = "axis")
anova(m2, by = "axis", perm.max=500)
is.na(plant_traits_MB)
```
...
ordination_scores <- as.data.frame(vegan::scores(m2dbRDA)$sites)
...
arrow_scores <- as.data.frame(vegan::scores(m2dbRDA, display = "bp"))
...
arrow_scores <- arrow_scores[sort(rownames(arrow_scores)),]
...
p_m2dbrda <- ggplot(data = merged_data, aes(x = dbRDA1, y = dbRDA2)) +
  ...
  geom_text(label_map, size = rel(4), data = A4.sub4, fontface = "bold", show.legend = FALSE, label = c("logLMA")) +
  geom_text(label_map, size = rel(4), data = A4.sub5, fontface = "bold", show.legend = FALSE, label = c("logLPS")) +
  geom_text(label_map, size = rel(4), data = A4.sub6, fontface = "bold", show.legend = FALSE, label = c("logLT")) +
...
A4.sub <- lapply(1:6, function(i) arrow_scores[i, ])

# dbRDA plot based on species
p_m2dbrda <- ggplot(data = merged_data, aes(x = dbRDA1, y = dbRDA2)) +
  geom_point(aes(color = Species, shape = Site), size = rel(4), alpha = 0.7, stroke = 1) +
  geom_segment(arrow_map, size = rel(.8), data = arrow_scores[1:2, ], color = "black", arrow = arrowhead) +
  geom_vline(xintercept = 0, color = "grey30", linetype = "dashed") +
  geom_hline(yintercept = 0, color = "grey30", linetype = "dashed") +
  geom_text(label_map, size = rel(4), data = A4.sub, fontface = "bold", show.legend = FALSE, label = c("Elev.", "logACI", "logLBI", "logLMA", "logLPS", "logLT")) +
  xlab(label = paste("dbRDA1 (", round(B$concont$importance[2, 1] * 100, digits = 1), "%)", sep = "")) +
  ylab(label = paste("dbRDA2 (", round(B$concont$importance[2, 2] * 100, digits = 1), "%)", sep = "")) + 
  scale_color_manual(values = met.brewer("Isfahan2", n = 3, type = "discrete", direction = -1)) +
  scale_shape_manual(values = c(1:22)) +
  stat_ellipse(aes(color = Species), geom = "path", size = 1.7, position = "identity", type = "t", linetype = 1, level = 0.95, segments = 51, na.rm = FALSE, show.legend = NA, inherit.aes = TRUE) +
  theme_classic(base_size = 16) +
  theme(legend.title = element_text(face = "bold"), legend.text = element_markdown()) +
  guides(shape = guide_legend(ncol = 2))

# Rearrange the order of layers so arrows are above the points
p_m2dbrda$layers <- c(p_m2dbrda$layers, p_m2dbrda$layers[[5]], p_m2dbrda$layers[[6]])
...
```{r, adonis2}
set.seed(123)
perma <- adonis2(rrfy_hell_matrix ~ logACI + 
                   Elevation_m + 
                   Site + 
                   Species,
...
```{r, SIMPER}
(sim <- with(ordi_traits, simper(asv_avgdist, Elevation_cat, permutations = 999)))
summary(sim)
```
