...
## Cleaning and Shaping leaf traits data
```{r, leaf_traits}
#| echo: false
#| eval: true
#| tidy: true
...
# 
# ## Leaf traits with community data
# plant_traits_comm <- plant_traits |>
...
lma <- ggplot(leaf_traits_noB, aes(y = logLMA, x = Species)) +
  ...
  scale_fill_manual(labels =c('*M. laciniatus*', '*M. nasutus*', '*M. guttatus*'),
                    values = met.brewer("Signac", n = 4, type = "discrete")) +
  scale_color_manual(aes(color = "black")) +
  #scale_color_manual(labels=c(''*M. laciniatus*', '*M. nasutus*', '*M. guttatus*'),values= met.brewer("Signac", n = 4, type = "discrete")) +
...
#### Elevation vs. Shannon diversity and Richness
...
#| fig-height: 5
#| label: elevation
#| fig-cap: "Scatter plots of leaf traits by elevation category."
...
summary(lm(Shannon ~ Elevation, data = plant_traits))
elev_div_p <- format.p(cor.test(plant_traits$Shannon, plant_traits$Elevation)$p.value) # This is just to make the p-value nicer. 


elev_diversity <- ggplot(data = plant_traits  |>
  filter(!Species == "B"), 
  aes(x = Elevation, y = Shannon, color = Species)) +
   ...
   scale_fill_manual(labels = c("*M. laciniatus*","*M. nasutus*", "*M. guttatus*"), 
                     values = met.brewer("Signac", n = 4, type = "discrete", direction = c(1))) + 
...
#Linear model summary
summary(lm(Observed ~ Elevation, data = plant_traits))
richness_p <- format.p(cor.test(plant_traits$Observed, plant_traits$Elevation)$p.value) # This is just to make the p-value nicer. 


rich_diversity <- ggplot(data = plant_traits  |>
  filter(!Species == "B"), 
  aes(x = Elevation, y = Observed, color = Species)) +
   geom_jitter(aes(color = plant_traits$Species[plant_traits$Species != "B"]), size = 3, alpha = 0.7) + 
   geom_smooth(method = lm, se = F) + 
   # stat_regline_equation(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = '~~~~'))) 
   geom_smooth(method = lm, se = T, level = 0.95, na.rm = F, color = "black") + 
...
pca_auto <- autoplot((pca), data = data.frame(leaf_traits.pca), # For some reason this functions require data to be data= data.frame()
  ...
  scale_color_manual(labels = c('*M. guttatus*', '*M. laciniatus*', '*M. nasutus*'),
                     values = met.brewer("Signac", n = 4, type = "discrete")) +
  stat_ellipse(aes(color = Species), geom = "path", size = 1.3, position = "identity", type = "t", linetype = 1, level = 0.95, segments = 51, na.rm = TRUE, show.legend = NA, inherit.aes = TRUE) +
  theme_classic(base_size = 12) +
...
#ggsave(filename = file.path(path, "Plots", "/pca_evo23.png"), plot = auto, dpi=300, units=c("mm"), width=150, height=150)
```
...
pp
p
#ggsave(file.path(path, "Plots", "/shannon_evo2023.png") , plot = pp, dpi=300, units=c("mm"), width=200, height=150)
