...
# Adding column and rownames
rownames(geo_distm) <- rownames(asv_avgdist)
colnames(geo_distm) <- rownames(asv_avgdist)
...
lac_mantel  <- mantel(asv_long |> # Subset the data
                     ...
                     geo_long |> # Subset the data
                       ...
                       column_to_rownames(var = "A") |>
                       as.matrix(),
                      method = "spearman", permutations = 9999, parallel = 8, na.rm = TRUE)
lac_mantel
...
set.seed(123)
anova_m3hell <- anova.cca(m3_hell, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
```
...
```{r}
# No site
m4_hell <- dbrda(dbrda_hell_matrix ~  logACI + logLT + Elevation_m + Species,  
            distance = "bray",
            dfun = vegdist,
            data = dbrda_traits,
            parallel = 8, #Passes parallelization to metaMDS function
...
summary(m4_hell)
set.seed(123)
anova.m4_hell <- anova.cca(m4_hell, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
...
# Anovas
set.seed(123)
anova_m4hell <- anova.cca(m4_hell, by = "margin", permutations = 999, parallel = 8) # test for sign. environ. variables
