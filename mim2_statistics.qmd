...
<!--
2026-08-28 (BAR): Added per-species lm(trait ~ Elevation_m) regressions for
logLMA, logLPS, and logLT (in the sections below the existing pooled
genus-wide models) to address collaborator revision item 1 -- reconciling
the Discussion's claim that LMA/LPS/LT increase with elevation "in all three
species" against the Results, which previously reported only a pooled trend.
See manuscript/collaborator_workplan_20260817.md for the full revision plan.
-->
...
# Functions
source("R/functions/mim2_functions.R")
...
# 00.1 Fast-Setup (post analyses)

All of these files are loaded here after I have cleaned and shaped the data throughout this notebook. The purpose of having them is to ease setting up the work space.
```{r}
#####################################################
############### Load master files here ##############
#####################################################
leaf_traits <- readRDS(file.path(
  path,
  "data/clean_data/statistics/leaf_traits.rds"
))

plant_traits <- readRDS(file.path(
  path,
  "data/clean_data/statistics/plant_traits.rds"
))

plant_traits_MB <- readRDS(file.path(
  path,
  "data/clean_data/statistics/plant_traits_MB.rds"
))

#ps_melt <- readRDS(file.path(path, "data/clean_data/statistics/ps_melt.rds"))
...
```{r}
#| label: phyloseq_objects

###################################
#### Load Phyloseq objects ########
###################################
# Load Phyloseq object cleaned of singletons (231 ASVs)
ps_clean_3 <- readRDS(file.path(
  path,
  "data/clean_data/taxonomy/TAXA_8450_phyloseq_nonsingletons_noB.rds"
))

ps_clean_3_df <- read.csv(file.path(
  path,
  "data/clean_data/taxonomy/TAXA_8450_phyloseq_nonsingletons_noB.csv"
))

pseq_rrfb <- readRDS(file.path(
  path,
  "data/clean_data/statistics/pseq_rrfb.rds"
))

#Mirlyn object from Method B
rarefied_phyloseq <- readRDS(file.path(
  path,
  "data/clean_data/statistics/rarefied_phyloseq.rds"
))
```

#### Community matrix, avgdist and geographic distance

```{r}
#| label: community_matrices

# Rarefied community matrix from "Matrices" section below.
rrfy_hell_matrix <- readRDS("data/clean_data/statistics/rrfy_hell_matrix.rds")

asv_avgdist <- readRDS("data/clean_data/statistics/asv_avgdist.rds") # Loading rarefied phyloseq object

geo_distm <- readRDS("data/clean_data/statistics/geo_distm.rds")
```
...
traits <- read.csv(file.path(
  path,
  "data/field_data/mim2_leaf_traits_field_survey.csv"
)) # Main raw data set
...
leaf_traits <- traits |>
  ...
  filter(!Site == "FVWB") |> # Removing out of bounds
  rename(
    LMA = LMA.mg.,
    LBI = Leaf_lobe_index,
    LPS = Leaf_toughness,
  ...
  mutate(
    ...
    Site = as.factor(Site),
    Elevation_cat = cut(Elevation_m, breaks = breaks, labels = labels)
  ) |>
...
Calculating Hill orders on non-rarefied data. The `estimate_richness()` is meant to be used with untrimmed data. It is highly dependnet on singletons. My data is already trimmed and decontaminated. My singleton "removal" eliminates ASVs with all zeros or samples wil all zeros. Singletons do remain, so this is not a problem.
Things to remember:
- Observed richness is Hill 0
- Simpson index is (1-D) (Hill 1)
- InvSimpson (1/D) (Hill 2)
...
```{r}
#| label: comm_diversity_leaf_traits

#### Leaf traits with community data ####

plant_traits <- ps_clean_3 |>
  estimate_richness(
    split = TRUE,
    measures = c("Observed", "Shannon", "Simpson")
  ) |>
  #  Simpson = 1 / Simpson) |>
  mutate(hill_1 = exp(Shannon), hill_2 = 1 / (1 - Simpson)) |>
  rownames_to_column() |>
  rename(Unique_ID = rowname) |>
  right_join(plant_traits, by = "Unique_ID") |>
  relocate(Observed, Shannon, Simpson, hill_1, hill_2, .after = logLMA) |>
...
ps_melt <- ps_clean_3 |>
  psmelt() |>
  rename(Unique_ID = Sample, ASV = OTU) |>
...
# Save file
# Leaf traits
# saveRDS(leaf_traits, file.path(path, "data/clean_data/statistics/leaf_traits.rds"))
# write.csv(leaf_traits, file.path(path, "data/clean_data/statistics/leaf_traits.csv"))
#
# # Plant traits with community data
# saveRDS(plant_traits, file.path(path, "data/clean_data/statistics/plant_traits.rds"))
# write.csv(plant_traits, file.path(path, "data/clean_data/statistics/plant_traits.csv"))
#
# # Save ps_melt
# saveRDS(ps_melt, file.path(path, "data/clean_data/statistics/ps_melt.rds"))
# write.csv(ps_melt, file.path(path, "data/clean_data/statistics/ps_melt.csv"))
```
...
## 02.2-Post data transformations
...
#Toughness
hist(
  leaf_traits$Toughness[leaf_traits$Toughness < 4],
  col = "steelblue",
  main = "Non-normal distribution"
)
hist(
  na.omit(leaf_traits$logLPS[leaf_traits$logLPS < 0.5]),
  col = 'coral2',
...
```{r}
#| label: non_parametric_tests

#Kruskal-Wallis tests

leaf_trait_vector <- c("logLBI", "logLMA", "logLPS", "logLT", "logACI")

non_parametric_tests <- data.frame(
  lf_traits = leaf_trait_vector
) |>
  mutate(
    site_test_pvalue = map(
      leaf_trait_vector,
      ~ kruskal.test(
        as.formula(paste0(., " ~ Site")),
        data = leaf_traits,
        na.action = na.omit
      ) |>
        broom::tidy() |>
    ...
    elev_test_pvalue = map(
      leaf_trait_vector,
      ~ kruskal.test(
        as.formula(paste0(., " ~ Elevation_m")),
        data = leaf_traits,
        na.action = na.omit
      ) |>
        broom::tidy() |>
...
# Added 2026-08-28: per-species LMA ~ Elevation regressions, to support/refute
# the Discussion's claim that LMA/LPS/LT increase with elevation "in all three
# species" (revision item 1, collaborator workplan). Previously only the
# pooled (genus-wide) model above existed.
leaf_traits_noB |>
  group_by(Species) |>
  group_modify(
    ~ broom::tidy(lm(logLMA ~ Elevation_m, data = .x)) |>
      filter(term == "Elevation_m")
  ) |>
  left_join(
    leaf_traits_noB |>
      group_by(Species) |>
      group_modify(~ broom::glance(lm(logLMA ~ Elevation_m, data = .x))) |>
      select(Species, r.squared, adj.r.squared, df.residual),
    by = "Species"
  )
...
lma_cont <- ggplot(leaf_traits_noB, aes(y = logLMA, x = Elevation_m)) +
  ...
  labs(
    y = "log10[Leaf Mass per Area (LMA) (mg/mm)]",
    x = "Elevation (m.a.s.l.)"
  ) +
...
#Comparisons
species_comparisons <- list(c("N", "L"), c("N", "G"), c("L", "G"))
...
lbi_cont <- ggplot(leaf_traits_noB, aes(y = logLBI, x = Elevation_m)) +
  ...
  theme_lfspp_continuous +
  labs(
    y = "log10[LBI (convex_hull - true_area]/convex_hull)]",
    x = "Elevation (m.a.s.l.)"
  ) +
  scale_y_continuous(expand = c(0.07, 0.07))
...
# Added 2026-08-28: per-species LPS ~ Elevation regressions (revision item 1;
# see note above the LMA per-species block).
leaf_traits_noB |>
  group_by(Species) |>
  group_modify(
    ~ broom::tidy(lm(logLPS ~ Elevation_m, data = .x)) |>
      filter(term == "Elevation_m")
  ) |>
  left_join(
    leaf_traits_noB |>
      group_by(Species) |>
      group_modify(~ broom::glance(lm(logLPS ~ Elevation_m, data = .x))) |>
      select(Species, r.squared, adj.r.squared, df.residual),
    by = "Species"
  )
...
lps_cont <- ggplot(leaf_traits_noB, aes(y = logLPS, x = Elevation_m)) +
  ...
  theme_lfspp_continuous +
  labs(
    y = expression(paste("Leaf punch strength (N mm", "^-1)")),
    x = "Elevation (m.a.s.l.)"
  ) +
  scale_y_continuous(expand = c(0.07, 0.07))
...
elev_lt_p <- format.p(
  cor.test(
    leaf_traits_noB$logLT,
    leaf_traits_noB$Elevation_m,
    na.action = na.omit
  )$p.value
) # This is just to make the p-value nicer.

# Added 2026-08-28: per-species LT ~ Elevation regressions (revision item 1;
# see note above the LMA per-species block).
leaf_traits_noB |>
  group_by(Species) |>
  group_modify(
    ~ broom::tidy(lm(logLT ~ Elevation_m, data = .x)) |>
      filter(term == "Elevation_m")
  ) |>
  left_join(
    leaf_traits_noB |>
      group_by(Species) |>
      group_modify(~ broom::glance(lm(logLT ~ Elevation_m, data = .x))) |>
      select(Species, r.squared, adj.r.squared, df.residual),
    by = "Species"
  )
...
lt_cont <- ggplot(leaf_traits_noB, aes(y = logLT, x = Elevation_m)) +
  ...
  theme_lfspp_continuous +
  labs(
    y = expression(paste("Leaf thickness (", mu, "m)")),
    x = "Elevation (m.a.s.l.)"
  ) +
  scale_y_continuous(expand = c(0.07, 0.07))
...
ggsave(
  filename = file.path(path, "Plots", "/lma_aci_evo23.png"),
  plot = arrange,
  dpi = 300,
  units = c("mm"),
  width = 300,
...
#LBI
ggsave(
  filename = file.path(path, "Plots", "/lbi_evo23.png"),
  plot = lbi,
  dpi = 300,
  units = c("mm"),
  width = 150,
...
ggsave(
  filename = file.path(path, "Plots", "/thg_thk_evo23.png"),
  plot = arrange2,
  dpi = 300,
  units = c("mm"),
  width = 300,
...
ant_arranged_with_caption <- annotate_figure(
  ant_arranged,
  bottom = text_grob(ant_caption),
...
pca_auto <- autoplot(
  (pca),
  data = data.frame(leaf_traits.pca), # For some reason this functions require data to be data= data.frame()
...
t <- ggplot(
  data = leaf_traits,
  aes(y = Thickness, x = Elevation_m, color = Species)
) +
...
a <- ggplot(
  data = leaf_traits,
  aes(y = ACI, x = Elevation_m, color = Species)
) +
...
tt <- ggplot(
  data = masterant2,
  aes(y = logit_herbivory, x = Thickness, color = E_load)
) +
...
lma <- ggplot(
  data = masterant2,
  aes(y = logit_herbivory, x = LMA, color = E_load)
) +
  geom_jitter(aes(color = E_load, shape = Species), size = 1.5, alpha = 0.7) +
  scale_shape_manual(values = c(15:22)) +
  geom_smooth(method = lm, se = F) +
...
dv <- ggplot(
  data = masterant2,
  aes(y = logit_herbivory, x = Shannon, color = E_load)
) +
...
#saveRDS(pseq_rrfa, file = "data/clean_data/statistics/pseq_rrfa.rds") # Saving rarefied phyloseq object
pseq_rrfa <- readRDS("data/clean_data/statistics/pseq_rrfa.rds") # Loading rarefied phyloseq object
...
plant_traits_MA <- alpha_rfy_DF(
  pseq_rrfa,
  diversity = c("shannon", "simpson", "invsimpson")
...
plant_traits_MA <- plant_traits_MA |>
  ...
  mutate(
    ...
    hill_1 = exp(shannon_)
  ) |> # Observed column is equivalent to hill_0
...
#saveRDS(plant_traits_MA, file = "data/clean_data/statistics/plant_traits_MA.rds") # Saving rarefied data frame
#write.csv(plant_traits_MA, file = "data/clean_data/statistics/plant_traits_MA.csv") # Saving rarefied data frame as csv

#Loading rarefied data frame
#plant_traits_MA <- readRDS("data/clean_data/statistics/plant_traits_MA.rds") # Loading rarefied data frame
```
...
methodB_ps <- prune_samples(
  sample_names(ps_clean_3) %in% methodB_samples,
  ps_clean_3
) # Selecting samples from randomization
...
# Save
#saveRDS(pseq_rrfb, file = "data/clean_data/statistics/pseq_rrfb.rds") # Saving rarefied phyloseq object

# Load
pseq_rrfb <- readRDS("data/clean_data/statistics/pseq_rrfb.rds") # Loading rarefied phyloseq object
...
ggplot() +
  geom_density(
    data = plant_traits_MA,
    aes(x = Observed, fill = Species),
    alpha = 0.5
...
ggplot() +
  ...
  geom_boxplot(
    ...
    aes(x = Species, y = Shannon, color = "yellow"),
    alpha = 0.1
  ) +
  theme_classic(base_size = 12) +
...
plant_traits_MB <- plant_traits_MB |>
  ...
  mutate(
    X = Unique_ID, # Copyin column to new column
    Unique_ID = str_remove(Unique_ID, "^.*?(_|-)"),
    hill_1 = exp(shannon_),
    logLBI = log10(LBI),
    logACI = log10(ACI),
...
# Saving rarefied data frame
#saveRDS(plant_traits_MB, file = "data/clean_data/statistics/plant_traits_MB.rds") # Saving rarefied data frame
#write.csv(plant_traits_MB, file = "data/clean_data/statistics/plant_traits_MB.csv") # Saving rarefied data frame as csv

#Loading rarefied data frame
plant_traits_MB <- readRDS("data/clean_data/statistics/plant_traits_MB.rds") # Loading rarefied data frame.
```
...
rrfy_asv_table <- as.data.frame(t(rep_otu_df(pseq_rrfb))) |> # Rarefied counts
  ...
  mutate(
    Unique_ID = X, # Copyin column to new column
    Unique_ID = str_remove(Unique_ID, "^.*?(_|-)")
  ) |>
...
TAXA <- read.csv(file.path(
  path,
  "data/clean_data/taxonomy/ASV_8450_phyloseq_nonsingletons_noB.csv"
)) # Taxonomic table
TAXA <- TAXA |>
  select(2:9) |>
...
#saveRDS(rarefied_phyloseq, file.path(path, "data/clean_data/statistics/rarefied_phyloseq.rds"))
...
rrfy_hell_matrix <- decostand(
  as.matrix(t(rep_otu_df(pseq_rrfb))),
  MARGIN = 1,
  method = "hellinger"
)
...
saveRDS(
  rrfy_hell_matrix,
  file = "data/clean_data/statistics/rrfy_hell_matrix.rds"
)
...
# Save
#saveRDS(asv_avgdist, file = "data/clean_data/statistics/asv_avgdist.rds") # Saving rarefied phyloseq object
...
# Save
saveRDS(geo_distm, file = "data/clean_data/statistics/geo_distm.rds")
...
hill0_diversity <- ggplot(
  ...
  geom_jitter(
    aes(color = Species), #stat = "summary",
    #fun = mean,
    size = 3,
    alpha = 0.3
  ) +
...
hill2_beta <- ggplot(
  plant_traits_MB,
  aes(x = Elevation_cat, y = hill_2, color = Elevation_cat)
) +
  ...
  guides(
    fill = guide_legend(title = "Elevation"),
    color = guide_legend(title = "Elevation")
  )
...
#### Models with only logLBI as leaf trait
```{r}
...
m2_hell <- dbrda(
  dbrda_hell_matrix ~ logLBI + Elevation_m + Sample_Date + Site + Species,
  distance = "bray",
  dfun = vegdist,
  data = dbrda_traits,
  parallel = 8, #Passes parallelization to metaMDS function
...
# No sampling date
m3_hell <- dbrda(
  dbrda_hell_matrix ~ logLBI + Elevation_m + Site + Species,
  distance = "bray",
  dfun = vegdist,
  data = dbrda_traits,
  parallel = 8, #Passes parallelization to metaMDS function
...
# Save
saveRDS(m3_hell, file = "data/clean_data/statistics/dbrda_m3_hell.rds")

# Load
m3_hell <- readRDS("data/clean_data/statistics/dbrda_m3_hell.rds")
...
saveRDS(
  anova_m3hell_margin,
  file = "data/clean_data/statistics/anova_m3hell_margin.rds"
)

#Load
anova_m3hell_margin <- readRDS(
  "data/clean_data/statistics/anova_m3hell_margin.rds"
)
```
...
# Save
saveRDS(m4_hell, file = "data/clean_data/statistics/dbrda_m4_hell.rds")

# Load
m4_hell <- readRDS("data/clean_data/statistics/dbrda_m4_hell.rds")
...
set.seed(123)
anova_m4hell_margin <-
  anova.cca(m4_hell, by = "margin", permutations = 999, parallel = 8)

# Save
saveRDS(
  anova_m4hell_margin,
  file = "data/clean_data/statistics/anova_m4hell_margin.rds"
)

#Load
anova_m4hell_margin <- readRDS(
  "data/clean_data/statistics/anova_m4hell_margin.rds"
)
...
# Save
saveRDS(m5_hell, file = "data/clean_data/statistics/dbrda_m5_hell.rds")
...
set.seed(123)
anova_m5hell_margin <-
  anova.cca(m5_hell, by = "margin", permutations = 999, parallel = 8)
saveRDS(
  anova_m5hell_margin,
  file = "data/clean_data/statistics/anova_m5hell_margin.rds"
)
anova_m5hell_margin <- readRDS(
  "data/clean_data/statistics/anova_m5hell_margin.rds"
)

# Terms (results are the same as adonis2 but without R2)
set.seed(123)
anova_m5hell_term <-
  anova(m5_hell, by = "term", permutations = 999, parallel = 8)
saveRDS(
  anova_m5hell_term,
  file = "data/clean_data/statistics/anova_m5hell_term.rds"
)
anova_m5hell_term <- readRDS("data/clean_data/statistics/anova_m5hell_term.rds")
...
saveRDS(beta_dis1, file = "data/clean_data/statistics/beta_dis1.rds")
beta_dis1 <- readRDS("data/clean_data/statistics/beta_dis1.rds")
anova(beta_dis1)

set.seed(123)
beta_perm1 <-
  permutest(beta_dis1, parallel = 8, permutations = 999, by = "term")
saveRDS(beta_perm1, file = "data/clean_data/statistics/beta_perm1.rds")
beta_perm1 <- readRDS("data/clean_data/statistics/beta_perm1.rds")
beta_perm1
...
saveRDS(beta_dis2, file = "data/clean_data/statistics/beta_dis2.rds")
beta_dis2 <- readRDS("data/clean_data/statistics/beta_dis2.rds")
# anova(beta_dis2)

set.seed(123)
beta_perm2 <-
  permutest(beta_dis2, parallel = 8, permutations = 999, by = "term")

saveRDS(beta_perm2, file = "data/clean_data/statistics/beta_perm2.rds")
beta_perm2 <- readRDS("data/clean_data/statistics/beta_perm2.rds")
beta_perm2
...
saveRDS(beta_dis3, file = "data/clean_data/statistics/beta_dis3.rds")
beta_dis3 <- readRDS("data/clean_data/statistics/beta_dis3.rds")


set.seed(123)
beta_perm3 <-
  permutest(beta_dis3, parallel = 8, permutations = 999, by = "term")

saveRDS(beta_perm3, file = "data/clean_data/statistics/beta_perm3.rds")
beta_perm3 <- readRDS("data/clean_data/statistics/beta_perm3.rds")

TukeyHSD(beta_dis3, conf.level = 0.95)
```

#### Model with all leaf traits

```{r}
# m1_lts with no interactions
m1_lts <- dbrda(
  dbrda_hell_matrix ~ logACI +
    logLT +
    logLPS +
    logLMA +
    logLBI +
...
saveRDS(m1_lts, file = "data/clean_data/statistics/m1_lts.rds")
m1_lts <- readRDS("data/clean_data/statistics/m1_lts.rds")
...
set.seed(123)
anova_m1_lts_margin <-
  anova.cca(m1_lts, by = "margin", permutations = 999, parallel = 8)

saveRDS(
  anova_m1_lts_margin,
  file = "data/clean_data/statistics/anova_m1_lts_margin.rds"
)
anova_m1_lts_margin <- readRDS(
  "data/clean_data/statistics/anova_m1_lts_margin.rds"
)

# Terms (results are the same as adonis2 but without R2)
set.seed(123)
anova_m1_lts_term <-
  anova.cca(m1_lts, by = "term", permutations = 999, parallel = 8)

saveRDS(
  anova_m1_lts_term,
  file = "data/clean_data/statistics/anova_m1_lts_term.rds"
)
anova_m1_lts_term <- readRDS("data/clean_data/statistics/anova_m1_lts_term.rds")

anova_m1_lts_term

#Saving all objects as a single .rda file
save(
  m3_hell,
  m4_hell,
  m5_hell,
  ...
  anova_m1_lts_margin,
  anova_m1_lts_term,
  file = "data/clean_data/statistics/dbRDA_models.rda"
)
```
...
p_final_model$layers <- c(
  p_final_model$layers,
  p_final_model$layers[[5]],
  p_final_model$layers[[6]]
)
p_final_model
...
set.seed(123)
perma2 <- adonis2(
  dbrda_hell_matrix ~ logACI + logLT + logLPS + logLMA + Elevation_m + Species,
  data = dbrda_traits,
  method = "bray",
  parallel = 8,
  permutation = 999,
...
save(perma3, file = "data/clean_data/statistics/perma3.rda")
load("data/clean_data/statistics/perma3.rda")
print(perma3)
```
...
save(pcoa_object, file = "data/clean_data/statistics/pcoa_object.rda")
load("data/clean_data/statistics/pcoa_object.rda")
...
pretty_percent <- format(
  round(explained_variance, digits = 1),
  nsma = 1,
  trim = TRUE
)
...
#ggsave(file.path(path, "figures/pcoa_dbrda.png"), plot = pcoa_dbrda_joined, dpi=300, units=c("mm"), width=300, height=300)
```


## Linear Regressions

### Preparing data for modelling GLMs - Impute missing values

```{r}
# Count columns with NA's
dbrda_traits |>
  summarise(across(everything(), ~ sum(is.na(.)))) |>
  t()

# There is lot of missing data in the leaf trait measurements.
# I will impute missing values based on the median due to non-normality of the data.
...
# Scaling data
# Not scaling data because the one I am using is the log transformed values of leaf traits.

# Splitting data into train and test data
# set.seed(1124)
# validationIndex <- createDataPartition(plant_traits_IMP$Unique_ID, p = 0.70, list=FALSE)
#
# train <- plant_traits_IMP[validationIndex,] # 70% of data to training
# test <- plant_traits_IMP[-validationIndex,] # remaining 30% for test
#
# head(train)
# head(test)
```

Not really pursuing the KNN classifier, although it might be a good option to explore an compare with dbRDA. 

- The resulting `dbrda_traits_imp` dataset can be used for dbRDA modelling.
...
elevation_pcoa1 <- ggplot(
  data = pcoa_merged_data,
  aes(x = Elevation_m, y = Dim1, color = Species)
) +
  geom_point(
    aes(color = Species),
    size = rel(4),
  ...
  labs(
    x = "Elevation (m)",
    y = paste("PCoA1 (", pretty_percent[1], "%)", sep = "")
  ) +
  stat_poly_eq(
    formula = formula,
    color = "black",
...
ggsave(
  file.path(path, "figures/elevation_pcoa1.png"),
  plot = elevation_pcoa1,
  dpi = 300,
  units = c("mm"),
  width = 300,
  height = 300
...
# elevation_pcoa2 <- ggplot(data = pcoa_merged_data, aes(x = Elevation_m, y = Dim2, color = Species)) +
#   geom_point(
#     aes(color = Species),
#     size = rel(4),
#     alpha = 0.2,
...
#   labs(x =  "Elevation (m)",
#        y = paste("PCoA2 (", pretty_percent[2], "%)", sep = "")) +
#   stat_poly_eq(
#     formula = formula,
#     color = "black",
#     aes(label = paste(
...
ggsave(
  file.path(path, "figures/pcoa_elevations.png"),
  plot = pcoa_elevations,
  dpi = 300,
  units = c("mm"),
  width = 300,
  height = 300
...
The simple linear model of PCoA 1 vs. Alpha diversity does not look good. When we plot the residuals, fitted values and Cook's distance we see it suffers from heteroscedasticity and outliers. The PCoA and dbRDa are qualitatively very similar. We observed overlapped of fungal communitites in *Mimulus* spp. A lot of the microbial literature presents PCoA, alphadiversity measures and PERMANOVA table. My issue with PCoA and doing a PCoR with a measure of alpha diversity or similar is that the model fit is not good at all. Visually it represents how at higher elevation we have lower alpha diversity levels. But this is information that is already present in the "boxplot_hill_orders" plots and the facetted dbRDA. I don't think the PCoA adds more information.

The PCoR of PCoA 1 vs. Elevation is a little bit more illustrative of the dynamics of the fungal communities. We see that the fungal communities are more similar at higher elevations. Maybe we can visualize this by plotting Bray-Curtis dissimilarity vs. Elevation. Similar to the Mantel tests.  

### Bray-Curtis vs. Elevation

Using the `dbrda_traits_imp` dataset for the simple regressions and GLMs. We will also add a new variable: `mean_bray` to the dataset, which is the mean Bray-Curtis dissimilarity for each species.
...
bray_elevation <- ggplot(
  ...
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Elevation (m)", y = "Mean Bray-Curtis dissimilarity") +
  scale_color_manual(
    labels = c("*M. laciniatus*", "*M. nasutus*", "*M. guttatus*"),
    values = met.brewer(
      "Isfahan2",
...
ggsave(
  file.path(path, "figures/bray_elevation.png"),
  plot = bray_elevation,
  dpi = 300,
  units = c("mm"),
  width = 300,
...
#### Joined PCoA 1 and Bray dissimilarity
```{r}
# Joined PCoA 1 and Bray dissimilarity

bray_pcoa_joined <- ggarrange(
  elevation_pcoa1,
  bray_elevation,
  nrow = 2,
  ncol = 1,
...
bray_pcoa_joined <- annotate_figure(
  bray_pcoa_joined,
  top = text_grob(
...
ggsave(
  file.path(path, "figures/bray_pcoa_joined.png"),
  plot = bray_pcoa_joined,
  dpi = 300,
  units = c("mm"),
  width = 300,
  height = 300
...
These regressions show that the mean bray-Curtis dissimilarity for each species decreases a elevation increases. FEF communities are more similar at higher elevations. It is a clearer way of visualizing the effects of elevation in the fungal communities.
...
# Stepwise selection of terms
B4AIC <- stepAIC(update(B4, . ~ ., -1), trace = TRUE, direction = "both", k = 2)
B4AIC$anova
...
# Model comparison
model.sel(B1, B2, B3, B4, B4AIC, B1REM, B1_gls)[, c(
  "df",
  "logLik",
  "AICc",
  "delta"
...
save(
  ...
  B4REM,
  B5REM,
  file = "data/clean_data/statistics/glmm_models.rda"
)

load("data/clean_data/statistics/glmm_models.rda")

#Model comparison and selection
model.sel(B1, B2, B3, B1REM, B3REM, B4AIC, B4REM, B5REM)[, c(
  "df",
  "logLik",
  "AICc",
  "delta"
...
model_plots <- plot_models(
  ...
  #theme_classic(base_size = 16) +
  ylim(-0.15, 0.15) +
  geom_hline(yintercept = 0, color = "grey30", linetype = "dashed") # plot vertical line
model_plots

ggsave(
  file.path(path, "figures/model_plots.png"),
  plot = model_plots,
  dpi = 300,
  units = c("mm"),
  width = 300,
...
test <- ggplot(test, aes(y = ASV_1, x = Elevation_m)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    y = "Cold-tolerant fungus (ASV_1)",
    x = paste("Elevation (m)", sep = "")
  ) +
  # scale_color_manual(
  #     labels = c("*M. laciniatus*", "*M. nasutus*", "*M. guttatus*"),
...
ggsave(
  file.path(path, "figures/cold-tolerant_asv1.png"),
  plot = test,
  dpi = 300,
  units = c("mm"),
  width = 250,
...
(sim <- with(
  dbrda_traits,
  simper(dbrda_hell_matrix, Species, permutations = 9999, parallel = 8)
))
summary(sim)
```
...
multipatt_analysis_ele <- multipatt(
  indiabun,
  elecat,
  func = "r.g",
  control = how(nperm = 999)
...
multipatt_analysis_sign <- as.data.table(
  multipatt_analysis$sign,
  keep.rownames = TRUE
)
...
# By Elevation
#Extract table of stats
multipatt_analysis_elev_sign <- as.data.table(
  multipatt_analysis_ele$sign,
  keep.rownames = TRUE
)
