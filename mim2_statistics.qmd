...
# #saveRDS(ftraits, file.path(path, "Statistics", "/ftraits.rds"))
# ftraits <- readRDS(file.path(path, "Statistics", "/ftraits.rds"))
```
...
#Toughness
hist(leaf_traits$Toughness[leaf_traits$Toughness < 4]) # Remove two outliers by using cut-off at 4.
mean(leaf_traits$Toughness[leaf_traits$Toughness < 4], na.rm = TRUE)
max(leaf_traits$Toughness[leaf_traits$Toughness < 4], na.rm = TRUE)
min(leaf_traits$Toughness[leaf_traits$Toughness < 4], na.rm = TRUE)
sd(leaf_traits$Toughness[leaf_traits$Toughness < 4], na.rm = TRUE)

qqPlot(leaf_traits$Toughness[leaf_traits$Toughness < 4]) 
hist(rnorm(n = length(leaf_traits$Toughness[leaf_traits$Toughness < 4]))) #For comparison: normal distribution based on random permutation of my data.
shapiro.test(leaf_traits$Toughness[leaf_traits$Toughness < 4]) #Since p-value is less than 0.05 this means the residuals from the Toughness data are not normally distributed. Data needs transformation.


#Thickness
hist(leaf_traits$Thickness)
mean(leaf_traits$Thickness, na.rm = TRUE)
max(leaf_traits$Thickness, na.rm = TRUE)
min(leaf_traits$Thickness, na.rm = TRUE)
sd(leaf_traits$Thickness, na.rm = TRUE)

qqPlot(leaf_traits$Thickness) # Skewed to the right. 
hist(rnorm(n = length(leaf_traits$Thickness))) #For comparison: normal distribution based on random permutation of my data.
shapiro.test(leaf_traits$Thickness) #Since p-value is less than 0.05 this means the residuals from the Thickness data are not normally distributed. Data needs transformation.
...
#Elevation
hist(leaf_traits$Elevation)
mean(leaf_traits$Elevation, na.rm = TRUE)
max(leaf_traits$Elevation, na.rm = TRUE)
min(leaf_traits$Elevation, na.rm = TRUE)
sd(leaf_traits$Elevation, na.rm = TRUE)

qqPlot(leaf_traits$Elevation) 
hist(rnorm(n = length(leaf_traits$Elevation))) #For comparison: normal distribution based on random permutation of my data.
shapiro.test(leaf_traits$Elevation) #Since p-value is less than 0.05 this means the residuals from the Thickness data are not normally distributed. Data needs transformation.
```

All leaf traits and elevation are not normally distributed. This requires transformation for further analysis. We will proceed weith a log transformation of the data.
...
# LBI by Elevation
print(kruskal.test(logLBI ~ Elevation, 
             data = leaf_traits,
             na.action = na.omit))
...
# LMA by Elevation
print(kruskal.test(logLMA ~ Elevation, 
                   data = leaf_traits,
                   na.action = na.omit))
...
# Toughness by Elevation
print(kruskal.test(logLPS ~ Elevation, 
                   data = leaf_traits,
                   na.action = na.omit))
...
# Thickness by Elevation
print(kruskal.test(logLT ~ Elevation, 
                   data = leaf_traits,
                   na.action = na.omit))
...
# ACI by Elevation
print(kruskal.test(logACI ~ Elevation, 
                   data = leaf_traits,
                   na.action = na.omit))
...
errbar_lims <- group_by(leaf_traits2, Site) |> 
...
#By Site
lma <- ggplot(leaf_traits2, aes(y= logLMA, x = Species)) +
...
ggsave(filename = file.path(path, "Plots", "/aci_evo23.jpg"), plot = aci, dpi=300, units=c("mm"), width=300, height=150)
```
...
lbi <- ggplot(leaf_traits2, aes(y= LBI, x = Species)) +
  geom_violin(aes(fill=Species, color = Species)) +
  geom_crossbar(stat="summary", fun = mean, fun.max = mean, fun.min = mean,  fatten = 2, width = 0.5) +
...
ggsave(filename = file.path(path, "Plots", "/lbi_evo23.jpg"), plot = lbi, dpi=300, units=c("mm"), width=300, height=150)
...
ggsave(filename = file.path(path, "Plots", "/thg_evo23.jpg"), plot = thg, dpi=300, units=c("mm"), width=300, height=150)
```
...
thk <- ggplot(leaf_traits2, aes(y= Thickness, x = Species)) +
  geom_violin(aes(fill=Species, color = Species)) +
  geom_crossbar(stat="summary", fun = mean, fun.max = mean, fun.min = mean,  fatten = 2, width = 0.5) +
  geom_point(color="black", alpha = 0.2,size =2, position = position_jitter(w=0.05)) +
    scale_fill_manual(labels=c('*M. guttatus*', '*M. laciniatus*', '*M. nasutus*'),values= met.brewer("Signac", n = 4, type = "discrete")) +
...
ggsave(filename = file.path(path, "Plots", "/thk_evo23.jpg"), plot = thk, dpi=300, units=c("mm"), width=300, height=150)
```
...
ggsave(filename = file.path(path, "Plots", "/lma_aci_evo23.jpg"), plot = arrange, dpi=300, units=c("mm"), width=300, height=150)

#LBI
ggsave(filename = file.path(path, "Plots", "/lbi_evo23.jpg"), plot = lbi, dpi=300, units=c("mm"), width=150, height=150)

#Toughness and Thickness
arrange2 <- ggarrange( thg, thk, nrow = 1, ncol=2, common.legend = TRUE, legend="right")
arrange2

ggsave(filename = file.path(path, "Plots", "/thg_thk_evo23.jpg"), plot = arrange2, dpi=300, units=c("mm"), width=300, height=150)
...
ggsave(filename = file.path(path, "Plots", "/elevation_evo2023.jpg"), plot = edv, dpi=300, units=c("mm"), width=250, height=150)
```
...
ggsave(filename = file.path(path, "Plots", "/rich_evo2023.jpg"), plot = rdv, dpi=300, units=c("mm"), width=250, height=150)
```
...
t <-ggplot(data = leaf_traits, aes(y= Thickness, x = Elevation, color = Species)) +
...
a <- ggplot(data = leaf_traits, aes(y= ACI, x= Elevation, color = Species))+
...
data.pca <- leaf_traits2[c(8:11, 14)]
data.pca <- leaf_traits2 |>
  na.omit() |>
  select(c(8:11,14)) |>
  rename(LT = Thickness, LPS = Toughness)

leaf_traits.pca <- leaf_traits2 |>
  na.omit()
...
auto <- autoplot((pca), data = data.frame(leaf_traits.pca), # For some reason this functions require data to be data= data.frame()
...
#ggsave(filename = file.path(path, "Plots", "/pca_evo23.jpg"), plot = auto, dpi=300, units=c("mm"), width=150, height=150)
```
...
# Richness data frame ####
rich <- estimate_richness(ps_clean_3, split = TRUE, measures = c("Observed","Shannon"))
...
pp1 
ggsave(file.path(path, "Plots", "/barplot_evo2023.jpg") , plot = pp1, dpi=300, units=c("mm"), width=200, height=150)
```
...
pp
p
ggsave(file.path(path, "Plots", "/shannon_evo2023.jpg") , plot = pp, dpi=300, units=c("mm"), width=200, height=150)
...
ggsave(filename=file.path(path, "Plots", "/dbRDA_evo23.svg"), plot=p_m3dbrda, dpi=600, units=c("mm"), width=150, height=150)
```
