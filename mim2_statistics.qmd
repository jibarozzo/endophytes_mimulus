...
This section cleans and shapes the raw data into two main data frames: `leaf_traits` and `plant_traits`. The first has all data point for leaf trait measurements at the leaf level (raw) and the second one represents the average per plant while incorporating FEF community data. The FEF community data is only available for 160/320 samples. For the downstream analyses, we eliminated *M. bicolor* or species "B" from the leaf trait data set and the phyloseq object. The removal of this is due to a small sample size for leaf trait measurements and even smaller for FEF community data (*n* = 3). The removal is not explicit in `plant_traits` it is only explicit in the subset `leaf_traits_noB` and and phyloseq objects. This is important for the rarefaction of sequence dept and later on in the `asv_matrix` that gives way for community analyses in [Community Diversity]. I include a custom function for p-value formatting.
...
# saveRDS(ps_clean_3, file.path(path, "clean_data/taxonomy/ASV_8450_assigned_nonsingletons_noB.rds"))
# ps_clean_3_df <- metagMisc::phyloseq_to_df(ps_clean_3, addtax = T, addtot = F, addmaxrank = F, sorting = "abundance")
#write.csv(ps_clean_3_df, file.path(path, "clean_data/taxonomy/ASV_8450_assigned_nonsingletons_noB.csv"))
...
ps_melt <- ps_clean_3 |>
  psmelt() |>
  filter(!sample_Species == "B") |> #Removing species B
  ...
  # ungroup() |>
  rename(Unique_ID = Sample) |>
  dplyr::select(!c(4, 6:15)) |>
  left_join(plant_traits |>
              dplyr::select(!Species), by = "Unique_ID") |>
  relocate(Kingdom:Species, .after = hill_2) #Reordering columns 
...
#Good's coverage
cover_goods <- ps_melt |>
  select(Unique_ID, sample_Species, OTU, Abundance) |>
  group_by(Unique_ID) |>
...
# Alpha diversity metrics (rarefied counts/abundance
# mirlyn package 
# 500 reads
# pseq_rrf5 <- mirl(ps_clean_3,
#                  libsize = 500,
#                  rep = 1000,
#                  set.seed = 433,
#                  trimOTUs = FALSE,
...
plant_traits_rf <- alpha_pseq |>
  ...
  select(
    -c(Observed.y, Shannon.y, hill_2, Simpson_1minusD)
  ) |> # Columns from data not rarefied
  mutate(hill_1 = exp(Shannon.x)) |> # Observed column is equivalent to hill_0
  ...
  rename(
    Observed = Observed.x, 
    Shannon = Shannon.x,
    Simpson_1minusD = Simpson,
    hill_2 = InvSimpson
  ) # Renaming columns
# This is a long pipe because I am removing information from the initial `plant_traits` data frame and joining with the rarefied data frame.
```
...
asv_matrix <- ps_clean_3_df |>
  select(!c(2:8)) |> 
  filter(Unique_ID %in% names_rarefy) |> # Filtering samples to match rarefied data frame 
...
set.seed(433)
asv_vegdist <- avgdist(t(asv_matrix), 
                       sample = 100,
                       dmethod = "bray",
                       meanfun = mean,
                       iterations = 1000,
...
m2 <- dbrda(asv_vegdist ~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_cat + Site + Species,  data = nmds_traits, na.action = na.omit)
m2
m2 <- na.omit(m2)
Anova(m2)
