...
#####################################################
leaf_traits <- readRDS(file.path(path, "data/clean_data/statistics/leaf_traits.rds")) 

plant_traits <- readRDS(file.path(path, "data/clean_data/statistics/plant_traits.rds"))

plant_traits_MB <- readRDS(file.path(path, "data/clean_data/statistics/plant_traits_MB.rds"))
...
ps_clean_3 <- readRDS(file.path(path, "data/clean_data/taxonomy/TAXA_8450_phyloseq_nonsingletons_noB.rds"))

ps_clean_3_df  <- read.csv(file.path(path, "data/clean_data/taxonomy/TAXA_8450_phyloseq_nonsingletons_noB.csv"))

pseq_rrfb <- readRDS(file.path(path, "data/clean_data/statistics/pseq_rrfb.rds"))

#Mirlyn object from Method B
rarefied_phyloseq <- readRDS(file.path(path, "data/clean_data/statistics/rarefied_phyloseq.rds"))
```
...
non_parametric_tests <- data.frame(
  ...
  mutate(
    site_test_pvalue = map(
      leaf_trait_vector,
      ~ kruskal.test(as.formula(paste0(., " ~ Site")), data = leaf_traits, na.action = na.omit) |>
        broom::tidy() |>
        pull(p.value) |>
    ...
    elev_test_pvalue = map(
      leaf_trait_vector,
      ~ kruskal.test(as.formula(paste0(., " ~ Elevation_m")), data = leaf_traits, na.action = na.omit) |>
        broom::tidy() |>
        pull(p.value) |>
        format.pval(digits = 5)
    )
...
#Taxonomixc table
TAXA <- read.csv(file.path(path,"data/clean_data/taxonomy/ASV_8450_phyloseq_nonsingletons_noB.csv")) # Taxonomic table
TAXA <- TAXA|>
  select(2:9) |>
...
# Save
saveRDS(rrfy_hell_matrix, file = "data/clean_data/statistics/rrfy_hell_matrix.rds") 
...
# Plot
hill0_diversity <- ggplot(data = plant_traits_MB, aes(x = Elevation_m, y = Observed, color = Species)) +
  geom_jitter(aes(color = Species), #stat = "summary",
              #fun = mean,
              size = 3, alpha = 0.3) +
  #geom_smooth(method = lm, se = F) +
  geom_smooth(
...
# Plot
hill1_diversity <- ggplot(data = plant_traits_MB, aes(x = Elevation_m, y = hill_1, color = Species)) +
  geom_jitter(aes(color = Species), #stat = "summary",
              #fun = mean,
              size = 3, alpha = 0.3) +
  geom_smooth(method = lm, se = F) +
...
m1_lts <- dbrda(
  dbrda_hell_matrix ~  logACI + logLT + logLPS + logLMA + logLBI + Elevation_m + Species,
  distance = "bray",
  dfun = vegdist,
  data = dbrda_traits,
  parallel = 8, #Passes parallelization to metaMDS function
...
saveRDS(anova_m1_lts_margin, file = "data/clean_data/statistics/anova_m1_lts_margin.rds")
anova_m1_lts_margin <- readRDS("data/clean_data/statistics/anova_m1_lts_margin.rds")
...
saveRDS(anova_m1_lts_term, file = "data/clean_data/statistics/anova_m1_lts_term.rds")
...
elevation_pcoa1 <- ggplot(data = pcoa_merged_data, aes(x = Elevation_m, y = Dim1, color = Species)) +
  geom_point(
    aes(color = Species),
    size = rel(4),
    alpha = 0.2,
...
ggsave(file.path(path, "figures/elevation_pcoa1.png"), plot = elevation_pcoa1, dpi=300, units=c("mm"), width=300, height=300)
...
ggsave(file.path(path, "figures/pcoa_elevations.png"), plot = pcoa_elevations, dpi=300, units=c("mm"), width=300, height=300)
```
...
# GGplot
bray_elevation <- ggplot(data = dbrda_traits, aes(x = Elevation_m, y = mean_bray, color = Species)) +
  geom_point(
    aes(color = Species),
    size = rel(4),
    alpha = 0.2,
...
ggsave(file.path(path, "figures/bray_elevation.png"), plot = bray_elevation, dpi=300, units=c("mm"), width=300, height=300)
```
...
bray_pcoa_joined <- ggarrange(elevation_pcoa1, bray_elevation,
nrow = 2,
ncol = 1,
common.legend = TRUE,
legend = "bottom",
...
ggsave(file.path(path, "figures/bray_pcoa_joined.png"), plot = bray_pcoa_joined, dpi=300, units=c("mm"), width=300, height=300)
```
...
save(B1, B2, B3, B4, B4AIC, B1REM, B3REM, B4REM, B5REM, file = "data/clean_data/statistics/glmm_models.rda")
