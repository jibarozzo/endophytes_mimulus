...
rownames_to_column(ps_clean_3@sam_data, var = "Unique_ID")

SAMP <- sample_data(plant_traits)
SAMP <- sample_data(plant_traits)

# Phyloseq object with cleaned ASV table
ps_clean_3 <- merge_phyloseq(ps_clean_3, SAMP)
...
ps_clean_3 %>%
  sample_names() %>%
  filter(ps_clean_3 %in% methodB_samples) 
prune_samples(sample_names(ps_clean_3) %in% methodB_samples, ps_clean_3)
```
