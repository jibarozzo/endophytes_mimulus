...
##### Compare rarefaction and rtandomization methods
```
...
transform_diver <- decostand(as.matrix(t(rep_otu_df(pseq_rrfb))), MARGIN = 1, method = "hellinger") # Hellinger transformation
        
# This is the Bray-Curtis dissimilarity matrix can be calculated when plotted. 
dist_pc <- vegdist(transform_diver, method = "bray")
pca<- prcomp(transform_diver, center = TRUE, scale = F)
pca$rotation=-pca$rotation
pca$x=-pca$x
biplot(pca)
# Previous 
...
m0 <- dbrda(asv_avgdist ~ 1, method = "bray", data = nmds_traits) #Model with intercept only.
m0
m0 <- na.omit(m0)
m1 <- dbrda(asv_avgdist ~ ., method = "bray", data = nmds_traits, na.action = na.omit) # Model with all explanatory variables.
m1 <- na.omit(m1)

# Model with species, site, leaf traits and elevation. ####
m2 <- dbrda(transform_diver~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_m + Site + Sample_Date+ Species, method = "bray", data = nmds_traits, na.action = na.omit)

m2 <- na.omit(m2)
Anova(m2)

#Anovas for m2
anova.cca(m2, by = "margin") 
anova(m2, by = "axis")
...
#m3
m2.forsel <-  ordistep(m0, scope = formula(m1), Pin = 0.5, Pout = 0.05 , trace = T, permutations = how(nperm = 999), steps = 50, direction = "forward", na.action = na.omit)
...
Ordistep fails due to missing values in the data. I will select manually. 

```{r}
# Remove logLBI p-value = 0.869
m3 <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + logLMA + Elevation_m + Sample_Date + Site + Species, method = "bray", data = nmds_traits, na.action = na.omit)
# Anovas
anova(m3, by = "margin") # test for sign. environ. variables

# Remove logLMA p-value = 0.754 
m4 <- dbrda(asv_avgdist ~ logACI + logLT + logLPS + Elevation_m + Sample_Date + Site + Species, method = "bray", data = nmds_traits, na.action = na.omit)
# Anovas
anova(m4, by = "margin") # test for sign. environ. variables

#Remove logLT p-value = 0.197
m5 <- dbrda(asv_avgdist ~ logACI + logLPS + Elevation_m + Sample_Date + Site + Species, method = "bray", data = nmds_traits, na.action = na.omit)
# Anovas
anova(m5, by = "margin") # test for sign. environ. variables

#Remove Sample_Date p-value = 0.207
m6 <- dbrda(asv_avgdist ~ logACI + logLPS + Elevation_m + Site + Species, method = "bray", data = nmds_traits, na.action = na.omit)
# Anovas
anova(m6, by = "margin") # test for sign. environ. variables

#Remove logLPS p-value = 0.124
m7 <- dbrda(asv_avgdist ~ logACI + Elevation_m + Site + Species, method = "bray", data = nmds_traits, na.action = na.omit)
# Anovas
...
p_m2dbrda <- ggplot(data = A.3, aes(x = dbRDA1, y = dbRDA2)) +
  geom_point(data = A.3, aes(color = Species, shape = Site), size = rel(4), alpha = 0.7, stroke = 1) +
  geom_segment(arrow_map, size = rel(.8), data = A.4[1:2,], color = "black", arrow = arrowhead) +
  ...
  geom_text(label_map, size = rel(4), data = A4.sub1, fontface = "bold", show.legend = FALSE, label = c("Elev.")) +
  geom_text(label_map, size = rel(4), data = A4.sub2, fontface = "bold", show.legend = FALSE, label = c("logACI")) +
  ...
  guides(color = FALSE, size = FALSE)guides(color = FALSE, size = FALSE)guides(color = FALSE, size = FALSE)
  
     
# Rearranging the order of layers so arrows are above the points.
p_m2dbrda$layers
p_m2dbrda$layers <- c(p_m2dbrda$layers, p_m2dbrda$layers[[5]], p_m2dbrda$layers[[6]])
p_m2dbrda
...
perma <- adonis2(asv_avgdist ~ logACI + 
                   ...
                   Species,
                 data = nmds_traits,
permutation = 9999, na.action = na.omit, 
strata = NULL)
print(perma)
...
beta <- betadisper(vegdist(t(asv_avgdist), method = "bray"), # This is a matrix of Hellinger transformed data rarefied 
                   nmds_traits$Species, 
                   type = "centroid", 
                   sqrt.dist = FALSE)
beta
