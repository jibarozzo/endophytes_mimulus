...
# Joining to plant traits_MB to avoid duplicate DFs 
plant_traits_MB <- left_join(plant_traits_MB, mean_bray, by = c("X"= "Unique_ID"))


# GGplot
bray_elevation <- ggplot(data = plant_traits_MB, aes(x = Elevation_m, y = mean_bray, color = Species)) +
  geom_point(
    aes(color = Species),
    size = rel(4),
    alpha = 0.2,
...
# Model with Elvation, Species and Site
B1 <- lm(
  mean_bray ~ Elevation_m + Species + Site + logACI + logLT + logLPS + logLMA + logLBI,
  data = plant_traits_MB,
...
#anova(B1,type="marginal") #What is a type III ANOVA?
Anova(B1, type = 3)
...
B2AIC <- stepAIC(update(B2, . ~ .), direction = "both", k = 2)
B2AIC$anova
...
anova(B3) #Adding Site as a random effect INCREASES AIC score.
Anova(B3, type = 3)
AIC(B3) # -14796.77

# Residuals histograms and plots ####
M2_resid <- resid(B3, type = "response")
M2_fitted <- fitted(B3)
...
# ANOVAs
Anova(B3, type = 3)
drop <- drop1(B3, . ~ ., test = "Chisq", trace = TRUE)
```
...
anova(B4) # Nesting Species in Site DECREASES AIC score.
AIC(B4) # -15633.11
...
```
The M4 model has a lower AIC score. We seem to have hit a good model. The fixed terms are `mean_bray ~ logACI + logLT + logLPS + logLMA + logLBI`. Let's take that an model random effects taking into account variance structure. 
...
B1REM <- lme(
  mean_bray ~ Elevation_m + logACI + logLT + logLPS + logLMA + logLBI,
  random = ~ 1 |
    Site,
  method = "REML",
  data = plant_traits_MB,
...
Anova(B1REM, type = 3)
AIC(B1REM) # -7849.631 This is a higher score than B1.
...
Anova(B1_gls, type = 3)
AIC(B1_gls) # -6815.453 is a higher AIC score. Site as a random effect is important.

plot_model(B1REM, type = "diag")

# Stepwise selection of terms
B1REM_drop <- drop1(B1REM, . ~ ., test = "Chisq", trace = TRUE)
```

The REML model with Elevation and Species as fixed terms has a higher AIC score than the `gls` model without random effects. Site as a random effect is important. Now, palying around with the method, I observed that "ML" models the data better than "REML". The AIC scores are lower. \
...
Anova(B3REM, type = 3)
AIC(B3REM) # -9527.507 is  a lower AIC score than M1REM
