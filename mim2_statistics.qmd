...
First let's create a dissimilarity and distance matrix.
```{r, dist_matrix}
#| echo: false
#| eval: true
#| tidy: true
...
pp <- ggplot(plant_traits |>
  ...
  stat_compare_means(comparisons = my_comparisons,
                     method = "wilcox.test", 
                     label = "p.signif") +
  labs(y = "Shannon Diversity", x = "", caption = "n = 160") +theme(axis.title.x = element_text(size = 14)) 
  facet_wrap(~Elevation_cat)
pp
...
m0 <- dbrda(asv_dist ~ 1, data = nmds_traits) #Model with intercept only.
m0
m0 <- na.omit(m0)
m1 <- dbrda(asv_dist ~ ., data = nmds_traits, na.action = na.omit) # Model with all explanatory variables.
m1

# Model with species, leaf traits and elevation. ####
m2 <- dbrda(asv_dist ~ logACI + logLT + logLPS + logLMA + logLBI + Elevation_m,  data = nmds_traits, na.action = na.omit)
m2
m2 <- na.omit(m2)
Anova(m2)
...
set.seed(123)
# By Species
beta <- betadisper(as.dist(asv_dist), nmds_traits$Species, type = "median", sqrt.dist = FALSE)
...
# By Elevation
beta2 <- betadisper(as.dist(asv_dist), nmds_traits$Elevation_cat, type = "centroid", sqrt.dist = FALSE)
anova(beta2)
TukeyHSD(beta2)
...
```{r, SIMPER}
(sim <- with(nmds_traits, simper(asv_dist, Elevation_cat, permutations = 999)))
summary(sim)
```
