...
##### Rafaction of sequences > samples (Method A)
...
#saveRDS(pseq_rrfa, file = "clean_data/statistics/pseq_rrf1.rds") # Saving rarefied phyloseq object
pseq_rrfa <- readRDS("clean_data/statistics/pseq_rrf1.rds") # Loading rarefied phyloseq object
...
plant_traits_rf <- alpha_rfy_DF(pseq_rrfa, diversity = c("shannon", "simpson", "invsimpson")) # Alpha diversity indices
...
plant_traits_MA <- plant_traits_rf |>
  ...
  filter(Unique_ID %in% random_samples) |># Selecting samples from randomization
  rename(
    Observed = observed, 
    Shannon = shannon_,
...
##### Rarefaction of samples > sequences (Method B)
...
rownames_to_column(ps_clean_3@sam_data@row.names, var = "Unique_ID")

SAMP <- sample_data(plant_traits)

# Phyloseq object with cleaned ASV table
ps_clean <- phyloseq(ASV, TAX, SAMP)
methodb_pseq <- ps_clean_3_df |>
  filter(Unique_ID %in% methodB_samples) |>
  column_to_rownames(var = "Unique_ID") |>
  row.names()# Rarefied phyloseq object)


pseq_rrfb <-  mirl(subset_samples(ps_clean_3, sample = methodB_samples),
                  libsize = 750,
                  rep = 50,
                  set.seed = 443,
                  trimOTUs = FALSE,
